...
UMAREN_SCORE_THRESHOLDS = [-100.0, -50.0, -25.0, 0.0, 10.0, 25.0, 50.0, 75.0, 100.0]
...
def cast_umaren_features(frame: pd.DataFrame, feature_columns: list[str]) -> pd.DataFrame:
...
def load_or_build_umaren_pair_frame(
    umaren_data_path: Path,
    raw_dir: Path,
    refresh: bool = False,
) -> pd.DataFrame:
    if umaren_data_path.exists() and not refresh:
        umaren_pairs = pd.read_csv(umaren_data_path, low_memory=False)
    else:
        umaren_data_path.parent.mkdir(parents=True, exist_ok=True)
        umaren_pairs = build_umaren_pair_label_frame(raw_dir=raw_dir)
        umaren_pairs.to_csv(umaren_data_path, index=False)
    if "RaceDate" in umaren_pairs.columns:
        umaren_pairs["RaceDate"] = pd.to_datetime(umaren_pairs["RaceDate"], errors="coerce")
    if "RaceKey" in umaren_pairs.columns:
        umaren_pairs["RaceKey"] = umaren_pairs["RaceKey"].astype(str)
    for column in ["Umaban1", "Umaban2"]:
        if column in umaren_pairs.columns:
            umaren_pairs[column] = pd.to_numeric(umaren_pairs[column], errors="coerce").astype("Int64")
    return umaren_pairs.sort_values(["RaceDate", "RaceKey", "Umaban1", "Umaban2"]).reset_index(drop=True)
...
def train_stage1_target_models(
    ...
    for target_col, score_col in UMAREN_STAGE1_TARGET_SPECS:
...
def build_umaren_candidate_frame(
    ...
    if top_k < 2:
        raise ValueError("top_k must be at least 2 for umaren pair generation.")
    if scored_df.empty:
        return pd.DataFrame()
    ...
    umaren_pairs = umaren_pair_frame.copy()
    if "RaceKey" in umaren_pairs.columns:
        umaren_pairs["RaceKey"] = umaren_pairs["RaceKey"].astype(str)
    for column in ["Umaban1", "Umaban2"]:
        if column in umaren_pairs.columns:
    ...
    right_columns = merge_columns + [
        column
        for column in umaren_pairs.columns
        if column not in merge_columns and column != "RaceDate"
    ]
    candidates = candidates.merge(
        umaren_pairs[right_columns].drop_duplicates(subset=merge_columns, keep="last"),
        on=merge_columns,
        how="left",
    )
    numeric_columns = UMAREN_NUMERIC_FEATURE_COLS + ["UmarenPayoff", "UmarenGrossReturn", "UmarenNetReturn", "UmarenHit"]
    for column in numeric_columns:
        if column in candidates.columns:
            fill_value = -100.0 if column == "UmarenNetReturn" else 0.0
            candidates[column] = _coerce_numeric(candidates[column], fill_value=fill_value)
    if "UmarenHit" in candidates.columns:
        candidates["UmarenHit"] = candidates["UmarenHit"].astype(int)
    return candidates.sort_values(["RaceDate", "RaceKey", "Horse1Top3Rank", "Horse2Top3Rank"], kind="stable").reset_index(drop=True)


def filter_tradable_umaren_candidates(candidate_df: pd.DataFrame) -> pd.DataFrame:
    if candidate_df.empty:
        return candidate_df.copy()
    if "UmarenOddsDecimal" not in candidate_df.columns:
        return candidate_df.copy()
    return candidate_df.loc[_coerce_numeric(candidate_df["UmarenOddsDecimal"]).gt(0)].reset_index(drop=True)


def build_umaren_feature_columns(frame: pd.DataFrame) -> list[str]:
    feature_columns = [column for column in UMAREN_NUMERIC_FEATURE_COLS if column in frame.columns]
    feature_columns.extend(
        column for column in UMAREN_CONTEXT_COLS if column in frame.columns and column not in feature_columns
    )
    return feature_columns


def fit_umaren_strategy_booster(
    ...
    train_data = lgb.Dataset(
        cast_umaren_features(train_df, feature_columns),
        label=_coerce_numeric(train_df["UmarenNetReturn"]),
        categorical_feature="auto",
    )
    eval_data = lgb.Dataset(
        cast_umaren_features(eval_df, feature_columns),
        label=_coerce_numeric(eval_df["UmarenNetReturn"]),
...
def train_final_umaren_strategy_booster(
    train_df: pd.DataFrame,
    feature_columns: list[str],
    model_path: Path,
    num_boost_round: int,
...
def build_selected_umaren_pairs(candidate_df: pd.DataFrame, score_col: str) -> pd.DataFrame:
    return (
        candidate_df.sort_values(
            ["RaceDate", "RaceKey", score_col, "UmarenNinki", "UmarenOddsDecimal", "PairTop3RankSum"],
...
def build_market_favorite_umaren_pairs(candidate_df: pd.DataFrame) -> pd.DataFrame:
    return (
        candidate_df.sort_values(
            ["RaceDate", "RaceKey", "UmarenOddsDecimal", "UmarenNinki", "PairTop3RankSum"],
...
def summarize_umaren_subset(picks: pd.DataFrame) -> dict[str, float]:
    bet_count = len(picks)
    stake = bet_count * 100.0
    gross_return = float(_coerce_numeric(picks.get("UmarenGrossReturn", pd.Series(dtype=float))).sum())
    pair_score_series = picks.get("PairTop3ScoreSum", picks.get("PairScoreSum", pd.Series(dtype=float)))
    return {
        "bets": bet_count,
        "umaren_hit_rate": float(_coerce_numeric(picks.get("UmarenHit", pd.Series(dtype=float))).mean()) if bet_count else 0.0,
        "umaren_return_rate": (gross_return / stake * 100.0) if stake else 0.0,
        "mean_predicted_net_return": float(_coerce_numeric(picks.get("UmarenStrategyScore", pd.Series(dtype=float))).mean())
        if bet_count
        else 0.0,
...
def summarize_umaren_baselines(candidate_df: pd.DataFrame) -> dict[str, dict[str, float]]:
    baselines = {
        "top3_sum": build_selected_umaren_pairs(candidate_df, "PairTop3ScoreSum"),
        "win_sum": build_selected_umaren_pairs(candidate_df, "PairWinScoreSum"),
        "market_favorite": build_market_favorite_umaren_pairs(candidate_df),
    }
    return {name: summarize_umaren_subset(frame) for name, frame in baselines.items()}


def build_umaren_thresholds(validation_picks: pd.DataFrame, score_col: str) -> list[float | None]:
    thresholds: set[float] = set(UMAREN_SCORE_THRESHOLDS)
...
def summarize_umaren_policy(
    ...
    validation_picks = build_selected_umaren_pairs(validation_candidates, score_col)
    test_picks = build_selected_umaren_pairs(test_candidates, score_col)
    thresholds = build_umaren_thresholds(validation_picks, score_col)
    minimum_bets = (
        min(
        ...
        rows.append(
            {
                ...
                "metrics": summarize_umaren_subset(validation_selected),
            }
        )
    ...
    best_validation = max(
        eligible,
        key=lambda row: (
            row["metrics"]["umaren_return_rate"],
            row["bet_count"],
            row["metrics"]["umaren_hit_rate"],
        ),
    )
    ...
    return {
        "selection_metric": "umaren_return_rate",
        ...
        "test_applied_threshold": {
            ...
            "metrics": summarize_umaren_subset(test_selected),
        },
    }


def build_umaren_workflow_gate(
    test_all_races_summary: dict[str, float],
    policy_summary: dict[str, object],
    threshold: float = 100.0,
) -> dict[str, float | bool]:
    policy_metrics = policy_summary.get("test_applied_threshold", {}).get("metrics", {})
    policy_return_rate = float(policy_metrics.get("umaren_return_rate", 0.0))
    all_races_return_rate = float(test_all_races_summary.get("umaren_return_rate", 0.0))
    return {
...
def build_oof_umaren_candidate_frame(
    train_df: pd.DataFrame,
    umaren_pair_frame: pd.DataFrame,
    drop_raw_ids: bool,
    top_k: int,
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for fold in folds:
            ...
            umaren_eval = umaren_pair_frame.loc[umaren_pair_frame["RaceKey"].isin(eval_race_keys)].copy()
            scored_fold, _, tuning_rounds = train_stage1_target_models(
                fold["train_df"],
                fold["eval_df"],
                model_dir,
                f"umaren_oof_{fold['label']}",
                drop_raw_ids=drop_raw_ids,
            )
            candidate_fold = build_umaren_candidate_frame(
                scored_fold,
                umaren_eval,
                "Top3Score",
                top_k=top_k,
            ...
            candidate_fold = filter_tradable_umaren_candidates(candidate_fold)
...
def score_temporal_period_umaren_candidates(
    history_df: pd.DataFrame,
    eval_df: pd.DataFrame,
    umaren_pair_frame: pd.DataFrame,
    output_dir: Path,
    ...
    candidates = build_umaren_candidate_frame(
        scored_df,
        umaren_eval,
        "Top3Score",
        top_k=top_k,
    ...
    return filter_tradable_umaren_candidates(candidates), tuning_rounds
...
def main() -> None:
    ...
    umaren_data_path = Path(args.umaren_data)
    umaren_pair_frame = load_or_build_umaren_pair_frame(
        umaren_data_path=umaren_data_path,
        raw_dir=Path(args.raw_dir),
        refresh=args.refresh_umaren_data,
    )
    if umaren_pair_frame.empty:
        raise ValueError("No umaren pair rows available.")
    ...
    print(
        f"Built OOF umaren candidates: {len(oof_candidates)} rows across {oof_candidates['RaceKey'].nunique() if not oof_candidates.empty else 0} races."
    )
    if oof_candidates.empty:
        raise ValueError("Umaren strategy OOF candidates are empty.")
    umaren_feature_columns = build_umaren_feature_columns(oof_candidates)
    ...
    validation_candidates, validation_stage1_rounds = score_temporal_period_umaren_candidates(
        train_df,
        validation_df,
        umaren_pair_frame,
        output_path.parent,
    ...
    print(
        f"Scored validation umaren candidates: {len(validation_candidates)} rows across {validation_candidates['RaceKey'].nunique() if not validation_candidates.empty else 0} races."
    )
    validation_candidates["UmarenStrategyScore"] = tuning_booster.predict(
        cast_umaren_features(validation_candidates, umaren_feature_columns)
    )

    final_train = pd.concat([oof_candidates, validation_candidates], ignore_index=True)
    final_booster = train_final_umaren_strategy_booster(
        final_train,
        umaren_feature_columns,
        final_model_path,
        tuning_rounds,
    )
    build_feature_metadata_path(final_model_path).write_text(
        json.dumps(
            {
                "feature_columns": umaren_feature_columns,
                "target_column": "UmarenNetReturn",
                "drop_raw_ids": args.drop_raw_ids,
                "top_k": args.top_k,
                "stage": "umaren_strategy",
                "oof_start_year": args.oof_start_year,
            },
            ensure_ascii=True,
            indent=2,
    ...
    test_candidates, test_stage1_rounds = score_temporal_period_umaren_candidates(
        final_history_df,
        test_df,
        umaren_pair_frame,
        output_path.parent,
    ...
    print(
        f"Scored test umaren candidates: {len(test_candidates)} rows across {test_candidates['RaceKey'].nunique() if not test_candidates.empty else 0} races."
    )
    test_candidates["UmarenStrategyScore"] = final_booster.predict(cast_umaren_features(test_candidates, umaren_feature_columns))

    validation_all_races_summary = summarize_umaren_subset(
        build_selected_umaren_pairs(validation_candidates, "UmarenStrategyScore")
    )
    test_all_races_summary = summarize_umaren_subset(
        build_selected_umaren_pairs(test_candidates, "UmarenStrategyScore")
    )
    policy_summary = summarize_umaren_policy(validation_candidates, test_candidates, "UmarenStrategyScore")

    result = {
        "data_path": str(data_path),
        "umaren_data_path": str(umaren_data_path),
        "drop_raw_ids": args.drop_raw_ids,
        "top_k": args.top_k,
        "oof_start_year": args.oof_start_year,
        ...
        "validation": {
            ...
            "baselines": summarize_umaren_baselines(validation_candidates),
            "all_races_umaren_strategy": validation_all_races_summary,
        },
        "test": {
            "candidate_rows": int(len(test_candidates)),
            "candidate_races": int(test_candidates["RaceKey"].nunique()),
            "baselines": summarize_umaren_baselines(test_candidates),
            "all_races_umaren_strategy": test_all_races_summary,
        },
        "policy": policy_summary,
        "workflow_gate": build_umaren_workflow_gate(test_all_races_summary, policy_summary),
    }
    output_path.write_text(json.dumps(result, ensure_ascii=True, indent=2), encoding="utf-8")
    print(json.dumps(result, ensure_ascii=False, indent=2))
    print(f"Saved umaren strategy summary to {output_path}")
